...
# ML_BOOTARGS_DEFAULT (the board's full mtdparts cmdline, with the active device's slot-B
# rootfs partition). drop_to_uboot_retry() calls the caller-defined sshg() for the
...
# The slot-B rootfs partition for ubi.mtd= comes from the active device profile as a NAME
# (ML_UBI_PARTITION, set by device.sh from board.conf's PARTITION). The kernel resolves it
# against the mtdparts names below, so the cmdline carries no partition index. Sourced here for
# callers that reach uboot.sh directly (ramboot-at-uboot.sh); re-sourcing device.sh after
# ssh-opts.sh already pulled it in is idempotent.
if [ -z "${ML_UBI_PARTITION:-}" ]; then
    # shellcheck source=/dev/null
    . "$_UBOOT_LIB_DIR/device.sh"
fi

# bootm does not get partitions from the dtb (unlike an SPL flash boot), so supply the full
# static mtdparts here.
