...
Record the same scene twice through the **same goggle**: once with a stock vendor air unit, once with ours. The goggle receives, decodes and re-encodes for the DVR, so that whole path is common to both legs and every difference between the two files came from the air side, which is the ISP configuration and the AE loop. Then measure the two files against each other offline.
