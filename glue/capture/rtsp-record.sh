...
# The codec is checked rather than assumed, and the pipeline below is built from it: recording a
# stream through the wrong depayloader produces a valid MP4 with no frames in it, and that
...
# pipeline's ML_DVR_CODEC (unset = the h264 default).
if [ -z "$CODEC" ]; then
...
case "$CODEC" in
    h264|h265) ;;
    *)
        echo "the pipeline is serving \"$CODEC\"; this records h264 or h265" >&2
        exit 1
        ;;
esac

URL="rtsp://$DEVICE_IP:554/venc8/stream"
echo "=== recording ${SECS}s from $URL ($CODEC) ==="
