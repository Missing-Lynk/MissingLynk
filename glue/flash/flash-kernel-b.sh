...
# Env:     DEVICE (device profile whose open-slot address is probed for the slot-B refusal)
#          DEVICE_IP (address to flash through; default $ML_STOCK_IP, where a stock-booted unit
#          always answers)
...
K_MTD="$(sshg 'grep -m1 "\"kernel1\"" /proc/mtd' | cut -d: -f1)"
D_MTD="$(sshg 'grep -m1 "\"dtb1\"" /proc/mtd' | cut -d: -f1)"
...
K0_MTD="$(sshg 'grep -m1 "\"kernel0\"" /proc/mtd' | cut -d: -f1)"
D0_MTD="$(sshg 'grep -m1 "\"dtb0\"" /proc/mtd' | cut -d: -f1)"
...
sshg "$STAGE/mtdtool erase /dev/$K_MTD && $STAGE/mtdtool write /dev/$K_MTD $STAGE/kernel1-container.bin"
echo "[*] mtdtool erase+write /dev/$D_MTD (dtb1, slot B)..."
sshg "$STAGE/mtdtool erase /dev/$D_MTD && $STAGE/mtdtool write /dev/$D_MTD $STAGE/dtb1.bin"
...
K_REMOTE_SHA="$(sshg "head -c $K_LOCAL_SIZE /dev/$K_MTD | sha256sum" | cut -d' ' -f1)"
D_REMOTE_SHA="$(sshg "head -c $D_LOCAL_SIZE /dev/$D_MTD | sha256sum" | cut -d' ' -f1)"
...
sshg "rm -f $STAGE/mtdtool $STAGE/kernel1-container.bin $STAGE/dtb1.bin"
