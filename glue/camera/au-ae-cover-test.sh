...
#
# The closed-loop validation: ml-3a runs at full authority (the recovered vendor law, no step
...
#
# The recorder is the streaming client; ml-3a only reads statistics through debugfs, so it is
...
echo "=== starting ml-3a at full authority ==="
sshg "rm -f /tmp/ml-3a.log
setsid /tmp/ml-3a --start-index $START_INDEX --verbose >/tmp/ml-3a.log 2>&1 &
sleep 2
head -2 /tmp/ml-3a.log" </dev/null || exit 1
...
sshg 'pkill -f ml-3a' </dev/null 2>/dev/null || true
device_pull /tmp/ml-3a.log "$OUT/ml-3a.log" || exit 1
...
awk '{ for (i = 1; i <= NF; i++) {
       ...
       if (NR == 1 || idx != last) { print "  " $0; last = idx } }' "$OUT/ml-3a.log" | head -40

echo "  ($(wc -l < "$OUT/ml-3a.log") decisions logged, $(awk '{for(i=1;i<=NF;i++) if($i=="step" && $(i+1) != 0) c++} END {print c+0}' "$OUT/ml-3a.log") of them moving the index)"
