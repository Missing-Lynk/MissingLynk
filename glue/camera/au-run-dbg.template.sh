...
HOOKLOG=$ML_TRACE_DIR/hook.log
{ echo "=== run_dbg $(date) ==="; } >> "$HOOKLOG" 2>/dev/null

# 1. ar_lowdelay PATH shim -> the REAL binary under LD_PRELOAD. The shim calls the
#    absolute real path so it can never recurse into itself even with the shim directory
...
mkdir -p "$ML_TRACE_DIR/bin" 2>/dev/null
cat > "$ML_TRACE_DIR/bin/ar_lowdelay" <<'SHIM' 2>/dev/null
#!/bin/sh
echo "ar_lowdelay hooked $(date)" >> /usrdata/missinglynk/mmiotrace/hook.log 2>/dev/null
exec env LD_PRELOAD=/usrdata/missinglynk/mmiotrace/mmiotrace.so MMIOTRACE_OUT=/tmp/mmio.log MMIOTRACE_LO=__LO__ MMIOTRACE_HI=__HI__ MMIOTRACE_READS=__READS__ MMIOTRACE_TIME=__TIME__ MMIOTRACE_NOMEM=__NOMEM__ MMIOTRACE_SKIP_LO=__SKIP_LO__ MMIOTRACE_SKIP_HI=__SKIP_HI__ MMIOTRACE_IOCTL_CENSUS=__CENSUS__ __REAL__ "$@"
SHIM
chmod +x "$ML_TRACE_DIR/bin/ar_lowdelay" 2>/dev/null
PATH=$ML_TRACE_DIR/bin:$PATH
export PATH
