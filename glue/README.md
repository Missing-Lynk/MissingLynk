...
- `camera/` targets the **air unit**, and is the one group that spans both slots: the bring-up and capture scripts run on open slot B, while `au-snapshot-vendor.sh`, `au-vendor-session.sh` and `au-slotA-mmiotrace.sh` read stock slot A. Every slot-A path is read-only. All of them source `lib/au-camera.sh`, which takes the slot-B address from the active device profile and refuses to run against a device that is not an air unit.
...
- `lib/` shared helpers used by the scripts here (and by `../userspace/hud/tools/deploy.sh`).
  - `ssh-opts.sh` the canonical SSH client option arrays: `SSH_OPTS_LIBRE` (open slot-B sshd) and `SSH_OPTS_LEGACY` (superset adding the legacy algorithms the vendor Dropbear on stock slot A / the air unit needs; harmless against the open sshd). Also the transport helpers every script shares (`sshg`, `device_ssh`, `device_ssh_timeout`, and the file transfers `device_push` / `device_push_as` / `device_pull`, which go over a plain SSH channel because neither stack has sftp) and the two reachability gates: `ensure_device_reachable` (open address, falling back to the stock unit) and `ensure_stock_slot_a` (the slot-A-only gate the flashers use). Sourcing it pulls in `device.sh`.
  ...
  - `au-camera.sh` the prologue the `camera/` scripts source instead of `ssh-opts.sh`: it maps the `AU_IP` / `AU_PASS` / `AU_PORT` overrides onto the transport, refuses to run when the active device is not an air unit (`RF_ROLE=air` in its `board.conf`), and provides `au_stock_slot_a` for the scripts that read the vendor system on slot A.
