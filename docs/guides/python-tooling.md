...
The boot hook gets a check of its own. `missinglynk/templates/*.sh` cannot be linted as they sit: they hold `@PLACEHOLDER@` tokens, so `$@NAME@` parses as the `$@` array and `@SKIP_MV_MIN@` is not a number. What matters is the file the device actually boots, so the suite renders the hook and runs both `sh -n` and shellcheck over the result. A syntax error there is a failed boot, and neither check may depend on reaching a device. The shellcheck half skips when shellcheck is not installed; CI runners have it, so it runs there. The rest of the shell tree is covered separately by `make check-shell` (see [the glue README](../../glue/README.md)).
...
- **A wedged device ends a command instead of hanging the CLI.** `run()`, `read_stream()` and `write_file()` all abort with a `TimeoutError` when the device goes quiet for `IDLE_TIMEOUT` (120 s, `Goggle(idle_timeout=...)` to change) - no output, no accepted bytes, no exit status. It measures silence, not total runtime, so a slow but progressing transfer is never cut off. The exit-status wait is bounded too, which is the case that matters when the USB gadget drops under a large push.
