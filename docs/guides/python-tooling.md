...
- **A wedged device ends a command instead of hanging the CLI.** `run()` aborts with a `TimeoutError` when a command produces no stdout, no stderr, and no exit status for `IDLE_TIMEOUT` (120 s, `Goggle(idle_timeout=...)` to change). It measures silence, not total runtime, so a slow command that keeps producing output is never cut off.
